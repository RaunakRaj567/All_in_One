...
def solve_cvrp_fallback_greedy(
    ...
    # Order vehicles starting with Truck 1 (20,000 kg) first to make Truck 1 fully operable & 100% loaded
    sorted_v_indices = list(range(num_vehicles))
    ...
    max_duration_mins = max((r["duration_minutes"] for r in routes), default=0.0)

    return {
        "status": "Optimal Routes Found",
        "total_distance_km": round(total_distance_km, 2),
        "total_duration_minutes": round(total_duration_mins, 1),
        "max_duration_minutes": round(max_duration_mins, 1),
